...
procedure TCLI.Print(const AText: string; const AArgs: array of const);
begin
  FLua.Print(AText, AArgs);
end;

procedure TCLI.PrintLn(const AText: string; const AArgs: array of const);
begin
  FLua.PrintLn(AText, AArgs);
end;

procedure TCLI.Execute(const AArgs: TArray<string>);
begin
...
procedure TCLI.ShowBanner;
begin
  PrintLn('Pyro Lua CLI Utility v%s', [PYRO_VERSION]);
  PrintLn('Copyright © 2024-present tinyBigGAMES™ LLC', []);
  PrintLn('All Rights Reserved.', []);
  PrintLn('', []);
  PrintLn('Usage:', []);
  PrintLn('  pcli <command> [arguments]', []);
  PrintLn('', []);
  PrintLn('Commands:', []);
  ...
  PrintLn('', []);
  PrintLn('Use "pcli <command> --help" for more information about a command.', []);
end;

procedure TCLI.ShowUsage(const ACommand: string);
begin
  ...
  begin
    ...
    PrintLn('', []);
    PrintLn('Usage:', []);
    PrintLn('  pcli run <script.lua>', []);
    PrintLn('', []);
    PrintLn('Arguments:', []);
    PrintLn('  <script.lua>    The Lua script to execute.', []);
    PrintLn('', []);
    PrintLn('Example:', []);
    PrintLn('  pcli run main.lua', []);
  end
...
procedure TCLI.RunScript(const AFilename: string);
var
  LFilename: string;
begin
  try
    ...
    FLua.UpdateArgs(2); // skip 'pcli run' on the commandline
...
procedure TCLI.InitProject(const AProjectName: string);
var
  LFileName: string;
  LIniFile: TIniFile;
  LResponse: string;
...
procedure TCLI.BuildProject(const AProjectName: string);
var
  LFileName: string;
  LIniFile: TIniFile;
  LSrcFilename: string;
